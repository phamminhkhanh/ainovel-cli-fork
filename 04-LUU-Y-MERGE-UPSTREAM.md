...
| **Advance gate trong sandbox** (2026-07-23) | Seed copy cả `meta/run.json` → sandbox thừa hưởng `advance_mode`. Nếu workspace chính từng `/review on`, job sẽ pause chờ `/next` mà web không gửi được → **treo vĩnh viễn**. `prepareRunDir` gọi `forceSandboxAutoAdvance(runOutDir)` ép `advance_mode=auto` + clear permit/hold **chỉ trong sandbox**. Không exclude `meta/run.json` khỏi seed được: `plan_start` là dữ kiện duy nhất để khôi phục crash giai đoạn quy hoạch, `pending_steer` là chủ đích. Nếu upstream đổi field advance trong `RunMeta`, cập nhật helper này. |
...
| **Workspace noise exclude** | Fingerprint/seed/sync-back phải dùng cùng exclude list: bỏ `logs/`, `*.log`, `diag/`, `diagnostics/`, `exports/`, temp/lock files. Nếu upstream thêm log/diagnostic path mới, cập nhật `shouldExcludeWorkspaceSeed`. **`meta/run.json` cố ý KHÔNG bị exclude** (cần `plan_start`/`pending_steer`) → phần advance gate được trung hòa sau seed, xem hàng "Advance gate trong sandbox". |
...
| **Advance gate trong sandbox** (2026-07-23) | Seed `continue_workspace` copy cả `meta/run.json` → sandbox thừa hưởng `advance_mode=review` / `advance_permit_chapter` / `advance_hold` nếu user từng gõ `/review on` trên workspace chính. Gate sẽ chặn mọi chương forward chờ `/next`, mà Cockpit **không có endpoint advance** → job treo vĩnh viễn. `prepareRunDir` gọi `forceSandboxAutoAdvance(runOutDir)` ép sandbox về `auto` + clear permit/hold (chỉ sandbox; host workspace không đổi, fingerprint giữ nguyên). Nếu upstream đổi tên field advance hoặc API `RunMeta`, cập nhật helper này. |
...
- [ ] Với advance gate: gõ `/review on` trên workspace chính (TUI) rồi tạo `continue_workspace` — sandbox `meta/run.json` phải về `advance_mode=auto`, permit 0, hold nil; `start_prompt`/`plan_start` phải còn; `run.json` của host **không** bị sửa (fingerprint không diverge).
...
| post-v0.6.1 (fork) | 2026-07-23 | Cockpit thích ứng engine mới: `waitProc` phân loại pause (`engine_paused`) thay vì gán nhầm `Hoàn thành`; marker `已暂停`; sandbox `continue_workspace` bị ép `advance_mode=auto` | Verify `runFinished`, `forceSandboxAutoAdvance`, marker list; test seed-from-review-workspace |
---
